...
export default function IssueResolverDashboard() {
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch("/api/reports?stage=pending_issue_resolver");
      if (response.ok) {
    ...
    try {
      const response = await fetch(`/api/reports/${reportId}/approve`, {
        ...
        body: JSON.stringify({
          action: "approve",
          note: note.trim(),
          nextStage: "pending_contractor",
        }),
      });

      if (response.ok) {
        alert("रिपोर्ट स्वीकृत और ठेकेदार को भेजी गई / Report approved and forwarded to Contractor");
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2 flex items-center gap-2">
          ...
          Issue Resolver Dashboard
        </h1>
        <p className="text-gray-600">स्वीकृत करें और ठेकेदार को भेजें / Review and forward to Contractor</p>
      </div>
