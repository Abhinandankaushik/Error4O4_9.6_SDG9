...
export default function InfraManagerDashboard() {
  ...
  const fetchReports = async () => {
    try {
      const response = await fetch("/api/reports?stage=pending_infra_manager");
      if (response.ok) {
    ...
    try {
      const response = await fetch(`/api/reports/${reportId}/approve`, {
        ...
        body: JSON.stringify({
          action: "approve",
          note: note.trim(),
          nextStage: "pending_issue_resolver",
        }),
      });

      if (response.ok) {
        alert("रिपोर्ट स्वीकृत और इश्यू रिज़ॉल्वर को भेजी गई / Report approved and forwarded to Issue Resolver");
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600">स्वीकृत करें और इश्यू रिज़ॉल्वर को भेजें / Review and forward to Issue Resolver</p>
      </div>
